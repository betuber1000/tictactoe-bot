...
async def tictactoe(interaction: discord.Interaction, opponent: discord.Member):
    ...
    await interaction.response.send_message(
        f"Game started between {interaction.user.mention} and {opponent.mention}!\n"
        f"{print_board(board)}\n"
        f"It is {interaction.user.mention}'s turn."
    )


@bot.tree.command(name="move", description="Place your mark on the board")
@app_commands.describe(row="Row (0-2)", column="Column (0-2)")
async def move(interaction: discord.Interaction, row: int, column: int):
    ...
    if not (0 <= row <= 2 and 0 <= column <= 2):
        await interaction.response.send_message("Row and column must be between 0 and 2.", ephemeral=True)
        return
    ...
    board[row][column] = game["symbols"][interaction.user.id]
